...
@section('title', 'Create Class Section - Student Information System')

@section('content')
<div class="page-header">
    <h1>Add New Class Section</h1>
</div>
...
<div class="card" style="max-width: 600px;">
    <form action="{{ route('class-sections.store') }}" method="POST">
        ...
        <div class="form-group">
            ...
            <input type="text" id="section_code" name="section_code" value="{{ old('section_code') }}" placeholder="e.g., A, B, C" required>
            @error('section_code') <span style="color: #dc3545; font-size: 0.875rem;">{{ $message }}</span> @enderror
        </div>

        <div class="form-group">
            <label for="schedule">Schedule *</label>
            <input type="text" id="schedule" name="schedule" value="{{ old('schedule') }}" placeholder="e.g., MWF 10:00-11:00" required>
            @error('schedule') <span style="color: #dc3545; font-size: 0.875rem;">{{ $message }}</span> @enderror
        </div>

        <div class="form-group">
            <label for="room">Room *</label>
            <input type="text" id="room" name="room" value="{{ old('room') }}" placeholder="e.g., A201" required>
        ...
        <div class="form-group">
            ...
            <input type="number" id="academic_year" name="academic_year" value="{{ old('academic_year') }}" min="2020" required>
        ...
        <div class="btn-group" style="justify-content: flex-start; gap: 1rem;">
            <button type="submit" class="btn btn-success">Create Class Section</button>
